...
from src.api.events import EventType, event_dispatcher
...
class EventBridge:
    ...
    def __init__(self):
        ...
        self.enabled = False
        self.stats = {"packets_processed": 0, "events_generated": 0, "errors": 0}
    ...
    async def _process_emoteto(self, packet: EmotetoPacket):
        ...
        event = event_dispatcher.create_event(
            EventType.EMOTETO_RECEIVED, event_data, priority=3, ttl=300
        )
    ...
    async def _process_channel_emote(self, packet: ChannelPacket):
        ...
        event = event_dispatcher.create_event(
            EventType.CHANNEL_EMOTE, event_data, priority=5, ttl=60
        )
    ...
    async def notify_mud_status(self, mud_name: str, online: bool, info: Optional[Dict] = None):
        ...
        event_data = {"mud_name": mud_name, "status": "online" if online else "offline"}

        if info:
            event_data["info"] = info

        event = event_dispatcher.create_event(event_type, event_data, priority=6, ttl=300)
    ...
    async def notify_channel_activity(
        self, channel: str, user: str, mud: str, action: str  # "joined" or "left"
    ):
        ...
        """
        event_type = (
            EventType.USER_JOINED_CHANNEL if action == "joined" else EventType.USER_LEFT_CHANNEL
        )

        event_data = {"channel": channel, "user": user, "mud": mud, "action": action}

        event = event_dispatcher.create_event(event_type, event_data, priority=7, ttl=60)
    ...
    async def notify_gateway_reconnect(self):
        ...
        event_data = {"message": "Gateway reconnected to I3 router", "status": "connected"}
