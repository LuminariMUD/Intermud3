        ...
        if self.gateway:
            ...
            if status["connected"]:
                status["connection"] = {
                    ...
                    "connected_at": self.gateway.connected_at.isoformat() if hasattr(self.gateway, "connected_at") else None,
                    "packet_stats": {
                        "sent": self.gateway.packets_sent if hasattr(self.gateway, "packets_sent") else 0,
                        "received": self.gateway.packets_received if hasattr(self.gateway, "packets_received") else 0
                    }
                }
            
            # Get service status
            status["services"] = {
                "tell": self.gateway.services.get("tell", 0) if hasattr(self.gateway, "services") else 0,
                "channel": self.gateway.services.get("channel", 0) if hasattr(self.gateway, "services") else 0,
                "who": self.gateway.services.get("who", 0) if hasattr(self.gateway, "services") else 0,
                "finger": self.gateway.services.get("finger", 0) if hasattr(self.gateway, "services") else 0,
                "locate": self.gateway.services.get("locate", 0) if hasattr(self.gateway, "services") else 0
            }
...
class StatsHandler(BaseHandler):
    ...
    async def handle(self, session: Session, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "session": session.metrics.to_dict()
        }
        ...
        if self.gateway:
            ...
            stats["gateway"] = {
                ...
                "mud_count": len(self.gateway.get_mudlist()) if hasattr(self.gateway, "get_mudlist") else 0,
                "channel_count": len(self.gateway.get_channel_list()) if hasattr(self.gateway, "get_channel_list") else 0
            }
    ...
    async def handle(self, session: Session, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.log_request(
                session, "reconnect", params, success,
                None if success else "Reconnection failed"
            )
...
class ReloadConfigHandler(BaseHandler):
    ...
    async def handle(self, session: Session, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        logger.info(f"Config reload requested by {session.mud_name} (session: {session.session_id})")
