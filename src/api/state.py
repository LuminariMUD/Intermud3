...
class ClientState:
    ...
    def __init__(self, session: Session, config: Optional[Dict] = None):
        ...
        # Message histories
        self.tell_history = MessageHistory(
            self.config.get('tell_history_size', 50)
        )
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            'channel_history': {
                name: hist.to_list()
                for name, hist in self.channel_history.items()
            },
    ...
    @classmethod
    def from_dict(cls, data: Dict, session: Session, config: Optional[Dict] = None) -> 'ClientState':
