...
from ..models.packet import (
    I3Packet, PacketType, TellPacket, EmotetoPacket,
    ErrorPacket, PacketValidationError
)
...
class TellService(BaseService):
    ...
    async def _handle_tell(self, packet: TellPacket) -> Optional[I3Packet]:
    ...
    async def _handle_emoteto(self, packet: EmotetoPacket) -> Optional[I3Packet]:
        ...
        if not user_session or not user_session.is_online:
            # User not online, send error reply
            self.logger.warning(
                "Emoteto target user not online",
                target_user=packet.target_user
            )
    ...
    async def send_tell(self, from_user: str, to_user: str, to_mud: str, 
        ...
        if success:
            self.logger.info(
                "Sent tell",
                from_user=from_user,
                to_user=f"{to_user}@{to_mud}"
            )
        
        return success
    
    async def send_emoteto(self, from_user: str, to_user: str, to_mud: str,
                          message: str, visname: Optional[str] = None) -> bool:
    ...
    def get_last_tell_sender(self, user: str) -> Optional[str]:
