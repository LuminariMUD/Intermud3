...
class BaseService(ABC):
    ...
    async def process_packet(self, packet: I3Packet) -> Optional[I3Packet]:
        ...
        try:
            ...
            if not await self.validate_packet(packet):
                logger.warning(f"Invalid packet for service {self.service_name}: {packet.packet_type}")
...
class ServiceRegistry:
        ...
        async with self._lock:
            ...
            for packet_type in service.supported_packets:
                if packet_type in self._packet_handlers:
                    self._packet_handlers[packet_type] = [
                        s for s in self._packet_handlers[packet_type]
    ...
    async def route_packet(self, packet: I3Packet) -> Optional[I3Packet]:
    ...
    async def shutdown_all(self) -> None:
        ...
        async with self._lock:
            ...
            shutdown_tasks = [
                service.shutdown() for service in self._services.values()
            ]
...
class ServiceManager:
    ...
    def get_metrics(self) -> Dict[str, ServiceMetrics]:
        ...
        """
        return {
            service.service_name: service.get_metrics()
