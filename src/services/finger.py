...
class FingerService(BaseService):
    ...
    async def _handle_finger_request(self, packet: I3Packet) -> Optional[I3Packet]:
    ...
    async def _handle_finger_reply(self, packet: I3Packet) -> Optional[I3Packet]:
        ...
        self.logger.info(
            "Received finger reply",
            from_mud=packet.originator_mud,
            for_user=packet.target_user
        )
    ...
    async def _get_user_info(self, username: str) -> Optional[Dict[str, Any]]:
        ...
        user_info = {
            ...
            "idle_time": idle_time,
            "ip_address": session.ip_address if not self.gateway or not self.gateway.settings.services.finger.get('hide_ip', True) else '',
            "level": session.level,
            "extra": {}
        }
    ...
    def _create_finger_reply(self, request: I3Packet, username: str, 
                           user_info: Optional[Dict[str, Any]]) -> I3Packet:
