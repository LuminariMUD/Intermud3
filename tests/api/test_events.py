...
from src.api.events import Event, EventDispatcher, EventFilter, EventType
...
class TestEvent:
    ...
    def test_event_to_json_rpc(self):
        ...
        event = Event(type=EventType.TELL_RECEIVED, data={"from_user": "Alice", "message": "Hello"})
    ...
    def test_event_expiry(self):
        ...
        # Non-expiring event
        event1 = Event(type=EventType.TELL_RECEIVED, data={}, ttl=None)
        assert not event1.is_expired()

        # Expired event
        event2 = Event(type=EventType.TELL_RECEIVED, data={}, ttl=1)
        event2.timestamp = datetime.utcnow() - timedelta(seconds=2)
        assert event2.is_expired()

        # Not expired event
        event3 = Event(type=EventType.TELL_RECEIVED, data={}, ttl=60)
...
class TestEventFilter:
    ...
    def test_filter_by_event_type(self, mock_session):
        ...
        filter_obj = EventFilter(event_types={EventType.TELL_RECEIVED, EventType.CHANNEL_MESSAGE})

        # Matching event
        event1 = Event(type=EventType.TELL_RECEIVED, data={"from_mud": "OtherMUD"})
        assert filter_obj.matches(event1, mock_session)

        # Non-matching event
        event2 = Event(type=EventType.MUD_ONLINE, data={"mud_name": "NewMUD"})
        assert not filter_obj.matches(event2, mock_session)

    def test_filter_by_channel(self, mock_session):
        """Test filtering by channel."""
        filter_obj = EventFilter(channels={"chat", "gossip"})

        # Matching channel
        event1 = Event(
            type=EventType.CHANNEL_MESSAGE, data={"channel": "chat", "from_mud": "OtherMUD"}
        )
        assert filter_obj.matches(event1, mock_session)

        # Non-matching channel
        event2 = Event(
            type=EventType.CHANNEL_MESSAGE, data={"channel": "admin", "from_mud": "OtherMUD"}
        )
        assert not filter_obj.matches(event2, mock_session)
    ...
    def test_filter_exclude_self(self, mock_session):
        ...
        # Event from same MUD
        event1 = Event(type=EventType.TELL_RECEIVED, data={"from_mud": "TestMUD"})
        assert not filter_obj.matches(event1, mock_session)

        # Event from different MUD
        event2 = Event(type=EventType.TELL_RECEIVED, data={"from_mud": "OtherMUD"})
...
class TestEventDispatcher:
    ...
    async def test_permission_checking(self, dispatcher, mock_session):
        ...
        event2 = Event(type=EventType.ERROR_OCCURRED, data={"error": "test"})
        assert dispatcher._should_send_event(mock_session, event2)

        # Remove 'tell' permission
        mock_session.permissions = {"channel"}
        event3 = Event(type=EventType.TELL_RECEIVED, data={"from_mud": "OtherMUD"})
    ...
    async def test_expired_event_handling(self, dispatcher, mock_session):
        ...
        event = Event(type=EventType.TELL_RECEIVED, data={"message": "old"}, ttl=1)
    ...
    async def test_custom_filter(self, dispatcher, mock_session):
        ...
        custom_filter = EventFilter(
            event_types={EventType.CHANNEL_MESSAGE}, channels={"chat"}, exclude_self=True
        )
        ...
        event2 = Event(
            type=EventType.CHANNEL_MESSAGE, data={"channel": "gossip", "from_mud": "OtherMUD"}
        )
        assert not dispatcher._should_send_event(mock_session, event2)
