...
from src.network.lpc import LPCDecoder, LPCEncoder, LPCError
...
class TestLPCEncoder:
    ...
    def test_encode_nested_mapping(self):
        ...
        test_dict = {"level1": {"level2": {"value": 42}}, "array": [1, 2, 3]}
        encoded = self.encoder.encode(test_dict)
        decoded = self.decoder.decode(encoded)
        assert decoded == test_dict
    ...
    def test_encode_complex_structure(self):
        ...
        complex_data = {
            "packet_type": "tell",
            "ttl": 200,
            "originator": {"mud": "TestMUD", "user": "testuser"},
            "target": {"mud": "TargetMUD", "user": "targetuser"},
            "message": "Hello, I3!",
            "metadata": [{"timestamp": 1234567890}, {"flags": ["urgent", "private"]}, None],
        }
...
class TestLPCDecoder:
    ...
    def test_roundtrip_all_types(self):
        ...
        test_cases = [
            ...
            {"key": "value"},
            b"bytes",
            {"mixed": [None, 42, "string", [1, 2, 3], {"nested": True}]},
        ]
