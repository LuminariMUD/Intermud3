...
def mock_session():
    ...
    session.has_permission = MagicMock(side_effect=lambda perm: perm in session_data["permissions"] or perm == "*")
    return session
...
class TestCommunicationHandler:
    ...
    async def test_tell_no_permission(self, handler, mock_session):
        ...
        mock_session.has_permission = MagicMock(side_effect=lambda perm: perm in mock_session.permissions or perm == "*")
...
class TestAdminHandler:
    ...
    async def test_get_mudlist(self, handler, mock_session):
        ...
        handler.gateway.get_mudlist = MagicMock(return_value={
            "TestMUD": {
                "host": "test.mud.com",
                "port": 4000,
                "status": "online"
