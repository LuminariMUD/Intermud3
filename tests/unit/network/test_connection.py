...
from src.network.mudmode import MudModeError, MudModeStreamProtocol
...
class TestRouterInfo:
    ...
    def test_router_info_creation(self):
        ...
        router = RouterInfo(name="test-router", address="test.example.com", port=8080)
    ...
    def test_router_info_with_priority(self):
        ...
        router = RouterInfo(name="fallback", address="fallback.example.com", port=8080, priority=1)
...
class TestConnectionManager:
    ...
    async def test_connect_primary_fails_fallback_succeeds(self):
        ...
        with (
            ...
            patch.object(manager, "routers", self.routers),
        ):  # Ensure router order
    ...
    async def test_connect_all_routers_fail(self):
        ...
        with (
            ...
            mock_set_state.assert_has_calls(
                [call(ConnectionState.CONNECTING), call(ConnectionState.ERROR)]
            )
    ...
    async def test_connect_router_backoff(self):
        ...
        with patch("time.time", return_value=1001.0):  # Just after attempt
            ...
            with (
                patch.object(self.primary_router, "can_attempt", return_value=False),
    ...
    async def test_disconnect(self):
        ...
        with patch.object(manager, "_set_state") as mock_set_state:
            ...
            mock_set_state.assert_has_calls(
                [call(ConnectionState.CLOSING), call(ConnectionState.DISCONNECTED)]
            )
    ...
    async def test_reconnect_task_execution(self):
        ...
        with patch("asyncio.sleep") as mock_sleep, patch.object(manager, "connect") as mock_connect:
    ...
    async def test_reconnect_task_while_closing(self):
        ...
        with patch("asyncio.sleep"), patch.object(manager, "connect") as mock_connect:
