...
class MockI3Router:
    ...
    async def send_mudlist_reply(self, writer):
        ...
        mudlist = [
            ...
            123456,  # ID
            {
                "TestMUD": {"ip": "127.0.0.1", "port": 4000, "mudlib": "Custom", "status": "open"},
                "OtherMUD": {"ip": "192.168.1.1", "port": 5000, "mudlib": "LP", "status": "open"},
            },
        ]
...
async def test_basic_connection():
    ...
    try:
        ...
        config = RouterConfig(primary=RouterHostConfig(host="127.0.0.1", port=9999), fallback=[])
...
async def test_failover():
    ...
    try:
        config = RouterConfig(
            primary=RouterHostConfig(host="127.0.0.1", port=9999),  # Primary not running
            fallback=[RouterHostConfig(host="127.0.0.1", port=9998)],
        )
...
async def test_connection_state_machine():
    ...
    try:
        config = RouterConfig(primary="*i3", address="127.0.0.1", port=9999, fallback=[])
...
async def test_packet_buffering():
    ...
    try:
        ...
        print("✓ All buffered packets sent after connection")
