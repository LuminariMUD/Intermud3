...
from src.config.models import MudConfig, RouterConfig, RouterHostConfig, ServiceConfig, Settings
...
def gateway_settings():
    ...
    return Settings(
        mud=MudConfig(name="TestGatewayMUD", port=4000, admin_email="admin@test.mud"),
        router=RouterConfig(primary=RouterHostConfig(host="127.0.0.1", port=8091)),
        gateway={"host": "127.0.0.1", "port": 8080, "auth": {"enabled": False}},
        services=ServiceConfig(tell=1, channel=1, who=1, finger=1, locate=1),
    )
