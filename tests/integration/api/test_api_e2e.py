...
async def test_websocket_authentication_flow():
    ...
    with patch.object(server.session_manager, 'authenticate', return_value=mock_session) as mock_auth:
        with patch('src.api.server.event_dispatcher') as mock_dispatcher:
...
async def test_event_flow_integration():
    ...
    from src.api.events import EventDispatcher, Event, EventType
