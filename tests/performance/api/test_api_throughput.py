...
def api_config():
    ...
    return APIConfig(host="127.0.0.1", port=8080)
...
class TestMessageThroughput:
        ...
        for i in range(message_count):
            if i % 3 == 0:
                ...
                message = {
                    ...
                    "params": {"channel": "chat", "message": f"Channel message {i}"},
                    "id": str(i),
                }
            else:
                # Who request
    ...
    async def test_concurrent_sessions_throughput(self, api_server):
        ...
        print(
            f"Processed {total_messages} messages from {session_count} sessions in {duration:.2f}s"
        )
        print(f"Throughput: {throughput:.2f} messages/second")
...
class TestEventThroughput:
    ...
    async def test_event_filtering_throughput(self):
        ...
        try:
            ...
            print(
                f"Processed {event_count} filtered events for {session_count} sessions in {duration:.2f}s"
            )
...
class TestSessionThroughput:
    ...
    async def test_session_creation_throughput(self):
        ...
        with patch.object(manager.auth, "authenticate") as mock_auth:
            ...
            assert (
                creation_throughput > 50
            ), f"Session creation too slow: {creation_throughput:.2f} sessions/s"
            assert (
                cleanup_throughput > 100
            ), f"Session cleanup too slow: {cleanup_throughput:.2f} sessions/s"
...
class TestMemoryEfficiency:
    ...
    async def test_message_queue_memory_efficiency(self, api_server, authenticated_session):
        ...
        print(
            f"Memory usage - Initial: {initial_memory:.1f}MB, Peak: {mid_memory:.1f}MB, Final: {final_memory:.1f}MB"
        )
    ...
    async def test_session_scaling_memory(self):
        ...
        with patch.object(manager.auth, "authenticate") as mock_auth:
            ...
            print(
                f"Session scaling - Initial: {initial_memory:.1f}MB, Peak: {peak_memory:.1f}MB, Final: {final_memory:.1f}MB"
            )
            ...
            assert (
                memory_per_session < 1.0
            ), f"Too much memory per session: {memory_per_session:.3f}MB"

            # Verify cleanup
            memory_increase = final_memory - initial_memory
            assert (
                memory_increase < 20
            ), f"Memory not properly cleaned up: {memory_increase:.1f}MB remaining"
