...
class TestLPCPerformance:
    ...
    def test_encoding_throughput(self):
        ...
        assert (
            ops_per_second > 5000
        ), f"Encoding throughput {ops_per_second} ops/s below target 5000"
    ...
    def test_decoding_throughput(self):
        ...
        assert (
            ops_per_second > 4000
        ), f"Decoding throughput {ops_per_second} ops/s below target 4000"
...
class TestServicePerformance:
    ...
    async def test_channel_service_latency(self):
        ...
        assert (
            stats["p99"] < 10
        ), f"Channel service P99 latency {stats['p99']}ms exceeds 10ms target"
    ...
    async def test_concurrent_service_handling(self):
        ...
        assert (
            ops_per_second > 1000
        ), f"Concurrent handling {ops_per_second} ops/s below target 1000"
    ...
    def test_memory_leak_detection(self):
        ...
        for iteration in range(5):
            ...
            if iteration > 2:  # Allow warm-up
                assert (
                    growth_mb < 10 * iteration
                ), f"Potential memory leak: {growth_mb:.2f}MB growth"
...
class TestCPUPerformance:
    ...
    def test_cpu_usage_encoding(self):
        ...
        assert (
            cpu_per_1000_ops < 50
        ), f"CPU usage {cpu_per_1000_ops:.1f}% per 1000 ops exceeds 50% target"
...
class TestEndToEndPerformance:
    ...
    async def test_throughput_target(self):
        ...
        # Test packets
        test_packets = [
            ["tell", 5, f"MUD{i%10}", f"user{i}", "Target", "user", f"Msg{i}"] for i in range(100)
        ]
        ...
        ), f"Throughput {packets_per_second} packets/s below 1000 target"
    ...
    async def test_concurrent_sessions(self):
        ...
        async def handle_session(session_id: int):
            ...
            for _ in range(10):
                ...
                packet = [
                    ...
                    "Target",
                    "user",
                    f"Hello from session {session_id}",
                ]
        ...
        assert (
            elapsed < 30
        ), f"Handling {num_sessions} sessions took {elapsed:.2f}s, exceeds 30s target"
