...
from src.services.channel import ChannelService, ChannelHistory, ChannelSubscription
from src.models.packet import (
    I3Packet, PacketType, ChannelMessagePacket
)
...
def sample_channel_message_packet():
    ...
    packet.to_lpc_array.return_value = [
        "channel-m", 200, "RemoteMUD", "sender", 0, 0, 
        "gossip", "Sender", "Hello channel!"
    ]
    return packet
...
def sample_channel_emote_packet():
    ...
    packet.originator_user = "sender"
    packet.to_lpc_array.return_value = [
        "channel-e", 200, "RemoteMUD", "sender", 0, 0,
        "gossip", "Sender", "waves happily"
    ]
    return packet
...
def sample_targeted_emote_packet():
    ...
    packet.originator_user = "sender"
    packet.to_lpc_array.return_value = [
        "channel-t", 200, "RemoteMUD", "sender", 0, 0,
        "gossip", "TestMUD", "receiver", "Sender", "Receiver", "pokes gently"
    ]
    return packet
...
class TestChannelMessageHandling:
    ...
    async def test_handle_channel_message(
        self, channel_service, sample_channel_message_packet,
        mock_state_manager
    ):
    ...
    async def test_handle_invalid_message_packet(
        self, channel_service, mock_state_manager
    ):
    ...
    async def test_message_filtering(
        self, mock_filter, channel_service, 
        sample_channel_message_packet, mock_state_manager
    ):
...
class TestChannelEmoteHandling:
    ...
    async def test_handle_channel_emote(
        self, channel_service, sample_channel_emote_packet
    ):
    ...
    async def test_handle_targeted_emote(
        self, channel_service, sample_targeted_emote_packet
    ):
...
class TestChannelSubscriptions:
    ...
    async def test_handle_channel_add(self, channel_service):
        ...
        packet.to_lpc_array.return_value = [
            "channel-add", 200, "RemoteMUD", "user1", 0, 0, "gossip"
        ]
    ...
    async def test_handle_channel_remove(self, channel_service):
        ...
        packet.originator_user = "user1"
        packet.to_lpc_array.return_value = [
            "channel-remove", 200, "RemoteMUD", "user1", 0, 0, "gossip"
        ]
    ...
    async def test_multiple_channel_subscriptions(self, channel_service):
        ...
        for channel in ["gossip", "chat", "ooc"]:
            ...
            packet.originator_user = "user1"
            packet.to_lpc_array.return_value = [
                "channel-add", 200, "RemoteMUD", "user1", 0, 0, channel
            ]
...
class TestChannelWho:
    ...
    async def test_handle_channel_who(self, channel_service):
        ...
        packet.to_lpc_array.return_value = [
            "channel-who", 200, "RemoteMUD", "requester", 0, 0, "gossip"
        ]
    ...
    async def test_channel_who_empty_channel(self, channel_service):
        ...
        packet.originator_user = "requester"
        packet.to_lpc_array.return_value = [
            "channel-who", 200, "RemoteMUD", "requester", 0, 0, "empty_channel"
        ]
...
class TestChannelListHandling:
    ...
    async def test_handle_chanlist_reply(
        self, channel_service, mock_state_manager
    ):
        ...
        packet.to_lpc_array.return_value = [
            "chanlist-reply", 200, "Router", 0, "TestMUD", 0, 12345,
            {
    ...
    async def test_handle_invalid_chanlist_reply(
        self, channel_service, mock_state_manager
    ):
...
class TestSendingMessages:
    ...
    async def test_send_message_default_visname(
...
class TestBroadcasting:
    ...
    async def test_broadcast_on_message(
        self, mock_broadcast, channel_service,
        sample_channel_message_packet, mock_state_manager
    ):
...
class TestEdgeCases:
    ...
    async def test_concurrent_subscriptions(self, channel_service):
        ...
        for i in range(10):
            ...
            packet.to_lpc_array.return_value = [
                "channel-add", 200, f"MUD{i}", f"user{i}", 0, 0, "gossip"
            ]
