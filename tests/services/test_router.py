...
from src.state.manager import MudInfo, StateManager
...
class TestLocalRouting:
    ...
    async def test_route_local_without_gateway(self, mock_state_manager, sample_tell_packet):
...
class TestBroadcastRouting:
    ...
    async def test_route_broadcast_without_gateway(self, mock_state_manager, sample_channel_packet):
    ...
    async def test_route_broadcast_integer_zero_target(self, router_service, mock_gateway):
...
class TestErrorHandling:
    ...
    async def test_send_error_reply_without_gateway(self, mock_state_manager, sample_remote_packet):
        """Test sending error reply without gateway."""
        service = RouterService(mock_state_manager, None)

        # Should not crash when gateway is None
        await service._send_error_reply(sample_remote_packet, "unk-dst", "No gateway")
    ...
    async def test_error_packet_includes_original_data(
        ...
        await router_service._send_error_reply(sample_remote_packet, "test-error", "Test error")
...
class TestStatistics:
    ...
    async def test_statistics_dropped_packets(self, router_service, sample_tell_packet):
...
class TestRoutingDestinations:
    ...
    async def test_route_to_same_mud_name(self, router_service, mock_gateway):
    ...
    async def test_route_determination_case_sensitivity(self, router_service, mock_gateway):
...
class TestConcurrentRouting:
    ...
    async def test_concurrent_local_routing(self, router_service, mock_gateway):
    ...
    async def test_concurrent_mixed_routing(
        ...
        local_packet = TellPacket(
            ttl=200,
            originator_mud="RemoteMUD",
            originator_user="sender1",
            target_mud="TestMUD",
            target_user="receiver1",
            message="Local",
        )
        ...
        remote_packet = TellPacket(
            ttl=200,
            originator_mud="SourceMUD",
            originator_user="sender2",
            target_mud="RemoteMUD",
            target_user="receiver2",
            message="Remote",
        )
...
class TestEdgeCases:
    ...
    async def test_route_packet_with_empty_target_mud(self, router_service, mock_state_manager):
