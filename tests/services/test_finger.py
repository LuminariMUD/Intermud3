...
from src.models.packet import (
    FingerPacket, ErrorPacket, PacketType, I3Packet
)
...
def sample_finger_request():
    ...
    packet.to_lpc_array.return_value = [
        "finger-req", 200, "RemoteMUD", "requester", "TestMUD", 0, "testuser"
    ]
...
def sample_finger_reply():
    ...
    packet.to_lpc_array.return_value = [
        "finger-reply", 200, "RemoteMUD", 0, "TestMUD", "requester", "testuser", "info"
    ]
...
class TestFingerRequestHandling:
    ...
    async def test_finger_request_invalid_packet_format(
    ...
    async def test_finger_request_empty_username(
        self, finger_service, mock_state_manager
    ):
    ...
    async def test_finger_request_case_insensitive(
        ...
        packet.target_user = ""
        packet.to_lpc_array.return_value = [
            "finger-req", 200, "RemoteMUD", "requester", "TestMUD", 0, "TESTUSER"  # Uppercase
        ]
    ...
    async def test_finger_request_with_number_username(
        self, finger_service, mock_state_manager
    ):
...
class TestUserInfoRetrieval:
    ...
    async def test_get_user_info_minimal_profile(
        self, finger_service, mock_state_manager
    ):
    ...
    async def test_get_user_info_nonexistent_user(
        self, finger_service, mock_state_manager
    ):
    ...
    async def test_idle_time_calculation(
        self, finger_service, mock_state_manager
    ):
...
class TestUtilityMethods:
    ...
    def test_create_finger_reply_with_user_info(self, finger_service, sample_finger_request):
        ...
        reply = finger_service._create_finger_reply(
            sample_finger_request, "testuser", user_info
        )
    ...
    def test_create_finger_reply_no_user_info(self, finger_service, sample_finger_request):
        ...
        reply = finger_service._create_finger_reply(
            sample_finger_request, "nonexistent", None
        )
...
class TestConcurrentOperations:
    ...
    async def test_concurrent_finger_requests(
        ...
        for i in range(5):
            ...
            packet.target_user = ""
            packet.to_lpc_array.return_value = [
                "finger-req", 200, f"MUD{i}", f"user{i}", "TestMUD", 0, "testuser"
            ]
...
class TestEdgeCases:
    ...
    async def test_missing_optional_session_attributes(
        self, finger_service, mock_state_manager
    ):
