...
class MockRouter:
    ...
    def _create_mock_mudlist(self) -> Dict[str, List[Any]]:
        ...
        return {
            "TestMUD": [
                ...
                {"tell": 1, "channel": 1, "who": 1, "finger": 1, "locate": 1},  # Services
                0,  # Other data
            ],
            "OtherMUD": [
                -1,
    ...
    def _create_mock_chanlist(self) -> Dict[str, Any]:
        ...
        """
        return {
            "imud_gossip": {"owner": "*i3", "type": 0},
            "imud_code": {"owner": "*i3", "type": 0},
            "test": {"owner": "TestMUD", "type": 1},
        }
    ...
    async def start(self):
        ...
        self.server = await loop.create_server(
            lambda: MudModeStreamProtocol(
                on_message=self._handle_message, on_connection_lost=self._handle_disconnect
            ),
            "127.0.0.1",
            self.port,
        )
    ...
    async def _handle_startup(self, packet: StartupPacket):
        ...
        reply = [
            ...
            0,
            [["*i3", "127.0.0.1 " + str(self.port)]],  # Router list
            self.mudlist_id,
        ]
    ...
    async def _handle_tell(self, packet: I3Packet):
        ...
        """
        print(
            f"Tell from {packet.originator_mud}:{packet.originator_user} to {packet.target_mud}:{packet.target_user}"
        )
    ...
    async def _handle_who_request(self, packet: I3Packet):
        ...
        who_reply = [
            ...
            [  # Mock user list
                {"name": "TestPlayer", "idle": 300, "level": 50, "extra": "Testing"}
            ],
        ]
